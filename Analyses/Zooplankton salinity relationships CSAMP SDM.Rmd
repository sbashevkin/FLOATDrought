...
zoop_data_mass<-zoop_data%>%
  ...
  st_join(deltamapr::R_DSIBM %>%
            select(SUBREGION)) %>%
  st_drop_geometry() %>% 
  filter(SUBREGION %in% c("NW Suisun","SW Suisun","NE Suisun","SE Suisun","Confluence"))%>%
...
newdata_function<-function(Station){
  ...
  mutate(Month=month(date),
         ...
         Station_fac=Station)%>% # Set arbitrary station (won't be used) 
  select(Month, doy, doy_s, Year_fac, Station_fac, SalSurf, SalSurf_s)
}

newdata[["Confluence"]]<-newdata_function(Station="513")
newdata[["East Suisun"]]<-newdata_function(Station="NZ048")
newdata[["NW Suisun"]]<-newdata_function(Station="NZ020")
newdata[["SW Suisun"]]<-newdata_function(Station="405")
...
sal_model<-function(group,region,new_data){
  par(mfrow=c(2,2))
  cat("<<<<<<<<<<<<<<<<<<<<<<< modeling", group, ">>>>>>>>>>>>>>>>>>>>>>>>>\n\n")
  
  model<-gam(BPUE_log1p ~ te(SalSurf_s, doy_s, k=c(5,5), bs=c("cs", "cc")) + 
               s(Year_fac, bs="re") + s(Station_fac, bs="re"),
             data=filter(zoop_data_mass, IBMR==group & Subregion_edit==region & Year>=2000), 
             method="REML")
  ...
  sal<-predict(model, type="terms", terms="te(SalSurf_s,doy_s)", newdata=new_data, se.fit=T)%>%
    as_tibble()%>%
...
##BM: I stopped here
sal_models_confluence<-map(set_names(unique(zoop_data_mass$IBMR)), sal_model(group=.x,region="Confluence",new_data=newdata[["Confluence"]]))
