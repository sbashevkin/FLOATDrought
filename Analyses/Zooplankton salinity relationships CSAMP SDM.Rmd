...
newdata_function<-function(region, data=zoop_data_mass){
  ...
  # Calculate monthly 95% quantiles of salinity
  month_sal<-data_filt%>%
    group_by(Month)%>%
    summarise(l95=quantile(SalSurf, 0.025),
              u95=quantile(SalSurf, 0.975))
  ...
  newdata<-expand_grid(date=mdy(paste(1:12, 15, 2001, sep="/")), # The 15th of each month on a non-leap year
    ...
    filter(SalSurf >= l95 & SalSurf <=u95)%>% # Remove any salinity values outside the 95% quantiles for each month
...
scenario_sal<-read_csv("Data/CSAMP_DS_SDM_salinity_scenarios.csv",
  ...
  mutate(Scenario=factor(Scenario, 
                         levels=c("sal_base","sal_fallX2_hi","sal_fallX2_low", 
                                  "sal_sumX2_hi","sal_sumX2_low","sal_SMSCG")),
